...
func TestTLSRouteWebhook_Default(t *testing.T) {
...
func TestTLSRouteWebhook_ValidateCreate(t *testing.T) {
	scheme := runtime.NewScheme()
	err := avapigwv1alpha1.AddToScheme(scheme)
	require.NoError(t, err)

	t.Run("valid TLSRoute with hostnames and rules", func(t *testing.T) {
	...
	t.Run("invalid - no hostnames", func(t *testing.T) {
		...
		_, err := webhook.ValidateCreate(context.Background(), route)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "at least one hostname is required")
	})
	...
	t.Run("invalid - invalid hostname format", func(t *testing.T) {
		cl := fake.NewClientBuilder().WithScheme(scheme).Build()
...
func TestTLSRouteWebhook_ValidateUpdate(t *testing.T) {
	scheme := runtime.NewScheme()
	err := avapigwv1alpha1.AddToScheme(scheme)
	require.NoError(t, err)
	...
	t.Run("valid update", func(t *testing.T) {
		...
		oldRoute := &avapigwv1alpha1.TLSRoute{
			ObjectMeta: metav1.ObjectMeta{Name: "test-route", Namespace: "default"},
			Spec: avapigwv1alpha1.TLSRouteSpec{
				ParentRefs: []avapigwv1alpha1.ParentRef{{
					Name: "test-gateway",
...
func TestTLSRouteWebhook_ValidateDelete(t *testing.T) {
	scheme := runtime.NewScheme()
	err := avapigwv1alpha1.AddToScheme(scheme)
	require.NoError(t, err)
